...
public class CookingPotTooltip implements ClientTooltipComponent
{
	...
	public void renderImage(Font font, int mouseX, int mouseY, PoseStack poseStack, ItemRenderer itemRenderer, int blitOffset) {
		...
		itemRenderer.renderAndDecorateItem(mealStack, mouseX, mouseY + textSpacing, 0);
	}
	...
	public void renderText(Font font, int x, int y, Matrix4f matrix4f, MultiBufferSource.BufferSource bufferSource) {
		...
		if (!mealStack.isEmpty()) {
			MutableComponent textServingsOf = mealStack.getCount() == 1
					? TextUtils.getTranslation("tooltip.cooking_pot.single_serving")
					: TextUtils.getTranslation("tooltip.cooking_pot.many_servings", mealStack.getCount());
			font.drawInBatch(textServingsOf, x, y, gray, true, matrix4f, bufferSource, false, 0, 15728880);
			font.drawInBatch(mealStack.getHoverName(), x + ITEM_SIZE + MARGIN, y + textSpacing + MARGIN, -1, true, matrix4f, bufferSource, false, 0, 15728880);
		} else {
			MutableComponent textEmpty = TextUtils.getTranslation("tooltip.cooking_pot.empty");
			font.drawInBatch(textEmpty, x, y, gray, true, matrix4f, bufferSource, false, 0, 15728880);
		}
	}
