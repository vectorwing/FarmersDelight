...
public class WildCropGeneration
{
	...
	public static void registerWildCropGeneration() {
		FEATURE_PATCH_WILD_CABBAGES = Registry.register(BuiltinRegistries.CONFIGURED_FEATURE, new ResourceLocation(FarmersDelight.MODID, "patch_wild_cabbages"),
				Feature.RANDOM_PATCH.configured(getWildCropConfiguration(ModBlocks.WILD_CABBAGES.get(), 96, 4, BlockPredicate.matchesBlock(Blocks.SAND, BLOCK_BELOW))));
		...
		FEATURE_PATCH_WILD_BEETROOTS = Registry.register(BuiltinRegistries.CONFIGURED_FEATURE, new ResourceLocation(FarmersDelight.MODID, "patch_wild_beetroots"),
				Feature.RANDOM_PATCH.configured(getWildCropConfiguration(ModBlocks.WILD_BEETROOTS.get(), 96, 4, BlockPredicate.matchesBlock(Blocks.SAND, BLOCK_BELOW))));
		...
		PATCH_WILD_ONIONS = Registry.register(BuiltinRegistries.PLACED_FEATURE, new ResourceLocation("patch_wild_onions"),
				FEATURE_PATCH_WILD_ONIONS.placed(RarityFilter.onAverageOnceEvery(150), InSquarePlacement.spread(), PlacementUtils.HEIGHTMAP, BiomeFilter.biome()));
		...
		PATCH_WILD_CARROTS = Registry.register(BuiltinRegistries.PLACED_FEATURE, new ResourceLocation("patch_wild_carrots"),
				FEATURE_PATCH_WILD_CARROTS.placed(RarityFilter.onAverageOnceEvery(150), InSquarePlacement.spread(), PlacementUtils.HEIGHTMAP, BiomeFilter.biome()));
		...
		PATCH_WILD_RICE = Registry.register(BuiltinRegistries.PLACED_FEATURE, new ResourceLocation("patch_wild_rice"),
				FEATURE_PATCH_WILD_RICE.placed(RarityFilter.onAverageOnceEvery(20), InSquarePlacement.spread(), PlacementUtils.HEIGHTMAP, BiomeFilter.biome()));
	}
}
